    ...
    return (
        <>
            ...
            <main className="ml-0 md:ml-[270px] flex-1 p-4 md:p-8 pt-[4.5rem] md:pt-8 min-h-screen">
                ...
                {/* Top Actions Bar */}
                <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 mb-6 md:mb-8">
                    <div className="relative flex-1">
                        ...
                        <input
                            ...
                            className="w-full bg-card border border-borderNormal rounded-xl py-2 sm:py-2.5 pr-4 pl-10 text-xs sm:text-sm focus:border-primary focus:ring-2 focus:ring-primaryGlow outline-none transition-all text-textMain"
                        />
                    </div>
                    <button
                        onClick={() => fetchApis(true)}
                        className="bg-card border border-borderNormal rounded-xl px-3 sm:px-4 py-2 sm:py-2.5 text-textSoft hover:text-textMain hover:border-borderLight transition-all flex items-center justify-center gap-2 text-xs sm:text-sm shrink-0"
                        title="Actualizar"
                    >
                    ...
                    <button
                        ...
                        className="bg-gradient-to-br from-primary to-purple-500 rounded-xl px-4 sm:px-5 py-2 sm:py-2.5 text-white font-bold text-xs sm:text-sm shadow-[0_4px_15px_rgba(99,102,241,0.3)] hover:shadow-[0_6px_20px_rgba(99,102,241,0.4)] hover:-translate-y-px transition-all flex items-center justify-center gap-1 sm:gap-2 shrink-0"
                    >
                ...
                {/* Filters Section */}
                <div className="bg-card border border-borderNormal rounded-2xl p-3 sm:p-4 mb-6">
                    <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
                        ...
                        <div className="flex bg-bg rounded-lg overflow-hidden border border-borderNormal">
                            {["all", "running", "stopped"].map((f) => (
                                <button
                                    key={f}
                                    onClick={() => setFilter(f)}
                                    className={`px-2 sm:px-4 py-1.5 sm:py-2 text-[11px] sm:text-xs font-semibold whitespace-nowrap transition-colors flex-1 sm:flex-none ${filter === f
                        ...
                        <div className="ml-auto flex items-center text-xs text-textMuted whitespace-nowrap">
                            <span className="text-[11px] sm:text-xs">{displayedApis.length}/{apis.length}</span>
                        </div>
                    </div>
                </div>
